...
func (p *Parser) UpdateTasksParse(ctx context.Context) (map[string][]Task, error) {
	...
	for _, cat := range allCats {
		...
		if pruned > 0 {
			log.Printf("korsars: updatetasksparse cat=%s maxPage=%d pruned=%d remaining=%d", cat, maxPages, pruned, len(merged))
		}
		p.tasks[cat] = merged
	}
...
func (p *Parser) ParseAllTask(ctx context.Context, force bool) (string, error) {
	...
	p.mu.Unlock()
	log.Printf("korsars: parsealltask cycle=%s pending=%d->%d/%d", cycle.CycleID, pendingAtStart, pendingLeft, cycleTotal)
	return "ok", nil
}
